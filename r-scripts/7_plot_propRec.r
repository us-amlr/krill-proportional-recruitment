...
fsh.m1 <- aggregate(fsh.m1$f,list(fsh.m1$Y),mean)
colnames(fsh.m1) <- c('Y','f')
#fsh.m2 <- fshry[fshry$M==2,]
#fsh.m2 <- aggregate(fsh.m2$f,list(fsh.m2$Y),mean)
#colnames(fsh.m2) <- c('Y','f')
# LTER penguins
ade.m1 <- ade[ade$mo==1,]
ade.m2 <- ade[ade$mo==2,]
...
# Palmer LTER trawl surveys, and the fishery observer data
  plt.name <- paste('plots_',juv.l,'mm/','Predators ',juv.l,'mm_48.1.pdf',sep='')
    pdf(file = plt.name)
    par(cex=1.3)
  ...
  plt.name <- paste('plots_',juv.l,'mm/AMLR & LTER trawls, Fishery 48.1 legs ',nlegs,'.pdf',sep='')
    pdf(file = plt.name)
    par(cex=1.3)
  plot(1992:2020,amlr.1$mean[match(1992:2020,amlr.1$yr)],
  main=paste('AMLR & LTER trawls, Fishery',sep=''), 
    xlab='Year',ylab = 'Proportional recruitment',ylim=y.lim,
    xlim=c(1991,2020),col='blue',pch=19)
...
fsh.agg <- fshry[fshry$M==1,]
fsh.agg <- aggregate(fsh.agg$f,list(fsh.agg$Y),mean)
colnames(fsh.agg) <- c('Y','f')
...
plt.name <- paste('plots_',juv.l,'mm/','Predators (COPA) & fishery (481.S) ',
                  juv.l,'mm_legs_',nlegs,'.pdf',sep='')
    pdf(file = plt.name)
    par(cex=1.3)
  ...
  plt.name <- paste('plots_',juv.l,'mm/','Predators (CAPE & LTER) & fishery (481.N) ',
                    juv.l,'mm_legs_',nlegs,'.pdf',sep='')
    pdf(file = plt.name)
    par(cex=1.3)
  ...
  legend(legend.placement[1],legend.placement[2],
    ...
    'fshry.481N'),lty=c(1),pch=c(19,19,19,19,17),merge=T,
    lwd=c(2,2,2,2,3),col=c('red','blue','brown','black'),cex=0.7)
  graphics.off()

  plt.name <- paste('plots_',juv.l,'mm/','AMLR and LTER trawls, Fishery legs',nlegs,'.pdf',sep='')
    pdf(file = plt.name)
    par(cex=1.3)
  plot(1992:2020,amlr.cs.1$mean[match(1992:2020,amlr.cs.1$yr)],
  main=paste('AMLR, LTER trawls and Fishery',sep=''), 
    xlab='Year',ylab = 'Proportional recruitment',ylim=y.lim,
    xlim=c(1991,2020),col='red',pch=19)
  ...
  points(1992:2020,lter.trwl[match(1992:2020,names(lter.trwl))],col='brown',pch=19)
  lines(1992:2020,lter.trwl[match(1992:2020,names(lter.trwl))],col='brown',lwd=2,lty=1)
  points(fsh.agg$Y,fsh.agg$f,col='black',pch=17)
  lines(fsh.agg$Y,fsh.agg$f,col='black',lwd=3)
