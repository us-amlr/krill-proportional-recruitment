...
if( nlegs == 1){ # combine 481S and 481N, Jan only
  Fm=summarise(group_by(Kmv,ASD,t),f=sum(fr[L<=juv.l]),M=1,Y=unique(Y))
...
if( nlegs == 2){ # combine 481S and 481N, Jan and Feb
  Fm=summarise(group_by(Kmv,ASD,t),f=sum(fr[L<=juv.l]),M=unique(M),Y=unique(Y))
  rec.prop.fshry <- as.data.frame(Fm)
